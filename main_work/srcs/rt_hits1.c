...
bool	rt_hit_sphere(t_ray *ray, t_object *sp)
{
	...
	a = rt_vector_length_squared(ray->dir); // normalement egal à 1
	half_b = rt_dot_prod(to_center, ray->dir);
	c = rt_vector_length_squared(to_center) - sp->radius * sp->radius;
